...
### 3.1.2 완료된 스킬을 패치할 때 Implementation Plan 관리
스킬의 모든 Phase가 끝나 Implementation Plan의 상태를 "완료"로 바꾼 뒤에는, 그 문서를 소급
수정하지 않고 최초 빌드 시점의 기록으로 그대로 둡니다(3.1.1의 "새로 시작하는 스킬에만 적용"
원칙과 같은 맥락). PRD/SRS는 패치 내용에 맞춰 바로 갱신하지만, Implementation Plan은 패치
때마다 아래 순서로 판단합니다.

1. 패치 자체에 3.1.1의 기준(외부 API 2개 이상 연동, 상태·재시도·롤백 로직, 선(先) 조사가
   필요한 불확실한 스펙, 한 세션에 끝내기 어려운 규모)을 다시 적용해봅니다.
2. 기준에 해당하지 않는 단순 패치는 Implementation Plan 없이 `PRD/SRS 갱신 → 코드 수정 →
   테스트 → CHANGELOG.md 기록` 순서로 바로 진행합니다(4절 체크리스트를 축소한 흐름).
3. 기준에 해당하는 규모의 패치라면, 기존 `docs/IMPLEMENTATION_PLAN.md`를 이어 쓰지 않고
   `docs/IMPLEMENTATION_PLAN_<날짜>_<패치 내용>.md`(예: `IMPLEMENTATION_PLAN_2026-08-06_retry.md`)
   형식으로 그 패치 범위만 다루는 새 문서를 추가합니다.

기존 파일에 패치 이력을 계속 이어 붙이지 않는 이유는, git 히스토리가 이미 시간순 변경 기록
역할을 하기 때문입니다. 패치 단위로 문서를 분리해두면 "이 패치가 왜 이렇게 설계됐는지"를
나중에 해당 파일 하나만 보고 파악할 수 있습니다.
